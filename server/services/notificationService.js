...
async function sendEmailNotification(userId, subject, body, ticketId) {
  try {
    ...
    const ticketLink = ticketId ? `${appUrl}/tickets/${ticketId}` : appUrl;
    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 24px; background: #ffffff; border: 1px solid #e5e7eb;">
            ...
            ${ticketId ? `<a href="${ticketLink}" style="display: inline-block; padding: 10px 24px; background: #667eea; color: white; text-decoration: none; border-radius: 6px; margin-top: 16px;">View Ticket</a>` : ''}
          </div>
      ...
      `,
      text: `${subject}\n\n${body}\n\n${ticketId ? `View ticket: ${ticketLink}` : ''}`,
    });
  } catch (error) {
