...
router.post('/', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    if (!asset_id || !title || !frequency || !next_due_date) {
      return res.status(400).json({ error: 'Asset, title, frequency, and next due date are required.' });
    }
...
router.post('/:id/complete', authenticate, async (req, res) => {
  try {
    const schedule = await db('preventive_maintenance').where({ id: req.params.id }).first();
    ...
    await db('preventive_maintenance').where({ id: req.params.id }).update({
    ...
    await db('assets').where({ id: schedule.asset_id }).update({
      last_serviced_at: new Date(),
      next_maintenance_date: nextDate.toISOString().split('T')[0],
      updated_at: new Date(),
    });

    const updated = await db('preventive_maintenance').where({ id: req.params.id }).first();
    res.json(updated);
  } catch (error) {
...
router.put('/:id', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    await db('preventive_maintenance').where({ id: req.params.id }).update(updates);
    const schedule = await db('preventive_maintenance').where({ id: req.params.id }).first();
    res.json(schedule);
  } catch (error) {
