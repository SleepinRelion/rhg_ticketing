...
router.post('/', authenticate, async (req, res) => {
  try {
    const { ticket_id, description, duration_minutes, parts_used, cost } = req.body;
    if (!ticket_id || !description) return res.status(400).json({ error: 'Ticket ID and description are required.' });

    const ticket = await db('tickets').where({ id: ticket_id }).whereNull('deleted_at').first();
    if (!ticket) return res.status(404).json({ error: 'Ticket not found.' });

    // Only assigned technicians can add interventions (or admin/manager)
    if (req.user.role === 'technician') {
      const isAssigned = await db('ticket_assignees').where({ ticket_id, user_id: req.user.id }).first();
      if (!isAssigned) return res.status(403).json({ error: 'You can only add interventions to tickets assigned to you.' });
    } else if (req.user.role === 'staff') {
      return res.status(403).json({ error: 'Staff cannot add interventions.' });
    }
    ...
    if (ticket.asset_id) {
      await db('assets').where({ id: ticket.asset_id }).update({ last_serviced_at: new Date(), updated_at: new Date() });
    }
...
router.get('/ticket/:ticketId', authenticate, async (req, res) => {
  try {
    const interventions = await db('interventions')
      .select('interventions.*', 'users.full_name as technician_name')
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    let query = db('interventions')
      .select('interventions.*', 'users.full_name as technician_name', 'tickets.ticket_number', 'tickets.title as ticket_title')
      .join('users', 'interventions.technician_id', 'users.id')
    ...
    const [{ count }] = await query.clone().clearSelect().clearOrder().count('interventions.id as count');
    const interventions = await query.orderBy('interventions.created_at', 'desc').limit(parseInt(limit)).offset(offset);

    res.json({ interventions, pagination: { page: parseInt(page), limit: parseInt(limit), total: parseInt(count) } });
  } catch (error) {
    console.error('List interventions error:', error);
    res.status(500).json({ error: 'Failed to fetch interventions.' });
  }
