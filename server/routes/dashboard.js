...
router.get('/charts', authenticate, asyncHandler(async (req, res) => {
  const {
    date_from,
    date_to
  } = req.query;
  ...
  // Active-only filter: excludes closed/cancelled tickets (for problem-area charts)
  let activeFilter = q => {
    q = q.whereNotIn('tickets.status', ['closed', 'cancelled']);
    return baseFilter(q);
  };
  ...
  const byCategory = await baseFilter(db('tickets').select(db.raw('COALESCE(parent.id, categories.id) as category_id'), db.raw('COALESCE(parent.name, categories.name) as category'), db.raw('count(*) as count')).leftJoin('categories', 'tickets.category_id', 'categories.id').leftJoin('categories as parent', 'categories.parent_id', 'parent.id').whereNotNull('tickets.category_id').groupByRaw('COALESCE(parent.id, categories.id), COALESCE(parent.name, categories.name)')).orderBy('count', 'desc');

  // Top 10 problem rooms
  const topRooms = await baseFilter(db('tickets').select('tickets.room_id', 'rooms.room_number', db.raw('count(*) as count')).join('rooms', 'tickets.room_id', 'rooms.id').groupBy('tickets.room_id', 'rooms.room_number')).orderBy('count', 'desc');

  // Top 10 problem assets
  const topAssets = await baseFilter(db('tickets').select('tickets.asset_id', 'assets.name as asset_name', db.raw('count(*) as count')).join('assets', 'tickets.asset_id', 'assets.id').groupBy('tickets.asset_id', 'assets.name')).orderBy('count', 'desc');

  // Top 10 problem room categories
  const topRoomTypes = await baseFilter(db('tickets').select('rooms.room_type as room_type', db.raw('count(*) as count')).join('rooms', 'tickets.room_id', 'rooms.id').whereNotNull('rooms.room_type').groupBy('rooms.room_type')).orderBy('count', 'desc');

  // Technician workload
  let techWorkloadQuery = db('ticket_assignees').select('users.id as user_id', 'users.full_name', db.raw('count(*) as count')).join('users', 'ticket_assignees.user_id', 'users.id').join('tickets', 'ticket_assignees.ticket_id', 'tickets.id').whereNotIn('tickets.status', ['closed', 'cancelled']).whereNull('tickets.deleted_at').where('users.is_active', true);
  techWorkloadQuery = buildTicketVisibilityQuery(techWorkloadQuery, req.user);
  const techWorkload = await techWorkloadQuery.groupBy('users.id', 'users.full_name').orderBy('count', 'desc');
  ...
  let monthlyTrendQuery = db('tickets').select(db.raw(`${monthFormat} as month`), db.raw('count(*) as count')).whereNull('deleted_at').where('created_at', '>=', sixMonthsAgo);
  monthlyTrendQuery = buildTicketVisibilityQuery(monthlyTrendQuery, req.user);
  const monthlyTrend = await monthlyTrendQuery.groupByRaw(monthFormat).orderBy('month');

  // SLA compliance
  const slaCompliance = await baseFilter(db('tickets').select('sla_status', db.raw('count(*) as count')).whereNotIn('status', ['cancelled']).groupBy('sla_status'));
  ...
  let weeklyInterventionsQuery = db('interventions').select(db.raw(`${weekFormat} as week`), db.raw('count(*) as count')).join('tickets', 'interventions.ticket_id', 'tickets.id').where('interventions.created_at', '>=', eightWeeksAgo);
  weeklyInterventionsQuery = buildTicketVisibilityQuery(weeklyInterventionsQuery, req.user);
  const weeklyInterventions = await weeklyInterventionsQuery.groupByRaw(weekFormat).orderBy('week');

  // PostgreSQL returns count as string (bigint); Recharts needs numbers.
  const parseCount = rows => rows.map(r => ({
    ...r,
