...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    if (!ticket_id || !content) return res.status(400).json({ error: 'Ticket ID and content are required.' });
    ...
    // Notify assignees
    if (!internal) {
      const assignees = await db('ticket_assignees').where({ ticket_id });
    ...
    if (req.io) {
      ...
      const updatedTicket = await db('tickets').where({ id: ticket_id }).first();
