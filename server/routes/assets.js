...
// GET /api/assets
router.get('/', authenticate, authorize('admin', 'manager', 'technician'), async (req, res) => {
  try {
    const { status, category_id, room_id, search } = req.query;
    let query = db('assets')
      .select('assets.*', 'categories.name as category_name', 'rooms.room_number')
...
router.get('/:id', authenticate, authorize('admin', 'manager', 'technician'), async (req, res) => {
  try {
    let query = db('assets')
      .select('assets.*', 'categories.name as category_name', 'rooms.room_number')
      .leftJoin('categories', 'assets.category_id', 'categories.id')
    ...
    const [openTickets, closedTickets, interventions, preventiveMaintenance] = await Promise.all([
      db('tickets')
      ...
      db('tickets')
        ...
        .limit(50),
      db('interventions')
        .select('interventions.*', 'users.full_name as technician_name', 'tickets.ticket_number')
        .join('users', 'interventions.technician_id', 'users.id')
        .join('tickets', 'interventions.ticket_id', 'tickets.id')
        .where('tickets.asset_id', asset.id)
        .orderBy('interventions.created_at', 'desc'),
      db('preventive_maintenance')
        .select('preventive_maintenance.*', 'users.full_name as assigned_to_name')
        .leftJoin('users', 'preventive_maintenance.assigned_to', 'users.id')
        .where('preventive_maintenance.asset_id', asset.id)
...
router.post('/', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    const data = req.body;
    if (!data.name || !data.asset_tag) return res.status(400).json({ error: 'Name and asset tag are required.' });
    ...
    if (error.code === '23505') return res.status(409).json({ error: 'Asset tag already exists.' });
...
router.post('/bulk', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, count: insertedAssets.length, assets: insertedAssets });
  } catch (error) {
    console.error('Bulk create asset error:', error);
...
router.put('/:id', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    const query = db('assets').where({ id: req.params.id });
...
router.delete('/:id', authenticate, authorize('admin', 'manager'), async (req, res) => {
  try {
    ...
    try {
      await db('assets').where({ id: req.params.id, hotel_id: hotelId }).del();
      res.json({ message: 'Asset deleted successfully.' });
    } catch (e) {
      if (e.code === 'SQLITE_CONSTRAINT' || e.code === '23503') {
        return res.status(409).json({ error: 'Cannot delete asset because it is referenced by existing tickets.' });
      }
      throw e;
    }
  } catch (error) {
